...
This report presents a comprehensive bioinformatics analysis of bladder cancer (TCGA-BLCA) RNA-seq data, focusing on differential gene expression, clustering analysis, and functional enrichment to identify molecular signatures distinguishing Low Grade (LG) from High Grade (HG) tumors. The analysis pipeline identified 28,023 genes after quality filtering and revealed 5,800 differentially expressed genes (DEGs) using DESeq2, highlighting critical biological pathways involved in cancer progression.

\subsection*{Key Findings:}
\begin{itemize}
    \item 5,800 DEGs identified using DESeq2 (FDR $<$ 0.01, $|\log_2\text{FC}| > 1$): 1,823 upregulated, 3,977 downregulated in High Grade tumors
    \item Optimal clustering: 2 clusters with high entropy (0.9816), indicating poor unsupervised separation of tumor grades
    \item PC1+PC2 explain 23.21\% variance on all genes, 38.32\% on DEGs only
    \item 247 significant GO:BP terms enriched, revealing immune response, proliferation, and ECM remodeling pathways
    \item Key methodological corrections: count-based filtering, DESeq2 instead of t-tests, proper entropy calculation
\end{itemize}
...
\begin{itemize}
    ...
    \item Features: 60,660 genes (initial) $\rightarrow$ 28,023 genes (after filtering)
...
Criteria Applied:
\begin{itemize}
    \item Minimum expression: raw count $>$ 5 in $\geq$10\% of samples (9 samples)
    \item Rationale: Count-based filtering removes lowly expressed genes without bias toward highly expressed genes. This approach ensures genes are excluded based on detection frequency rather than relative abundance.
\end{itemize}

Results:
\begin{itemize}
    \item Original: 60,660 genes
    \item Retained: 28,023 genes (46.1\%)
    \item Removed: 32,637 genes (53.8\%)
\end{itemize}
...
Implementation:
\begin{itemize}
    \item Performed on $\log_2$CPM values (28,023 genes)
...
Results:
\begin{itemize}
    \item Cluster 0: 89 samples (98.9\%)
    \item Cluster 1: 1 sample (1.1\%)
    \item Silhouette Score: 0.3088 (optimal k=2)
\end{itemize}
...
\midrule
Optimal k & 2 & 2 \\
Silhouette Score (k=2) & 0.3088 & 0.2959 \\
Total Entropy & 0.9816 & -- \\
\bottomrule
\end{tabular}
\end{table}

Conclusion: Both methods identified k=2 as optimal. K-means had slightly higher silhouette score. High entropy (0.9816) indicates poor separation of tumor grades by unsupervised clustering.
...
\midrule
Cluster 0 & 49 (55.1\%) & 40 (44.9\%) & 89 & 0.9926 \\
Cluster 1 & 1 (100\%) & 0 (0\%) & 1 & 0.0000 \\
\bottomrule
\end{tabular}
\end{table}

Total Weighted Entropy: 0.9816 (0 = perfect, 1 = random)

Interpretation:
\begin{itemize}
    \item Cluster 0 contains almost all samples with near-maximal entropy (poor separation)
    \item Cluster 1 is a single-sample outlier cluster
    \item High entropy (0.9816) indicates unsupervised clustering cannot separate tumor grades effectively
    \item Motivates need for supervised differential expression analysis
\end{itemize}
...
\midrule
Total Tested & 28,023 & 100\% \\
FDR $<$ 0.01 \& $|\log_2\text{FC}| > 1$ & \textbf{5,800} & \textbf{20.7\%} \\
Upregulated (HG) & 1,823 & 31.4\% \\
Downregulated (HG) & 3,977 & 68.6\% \\
\bottomrule
\end{tabular}
\end{table}
...
Total Significant Terms: 247 GO:BP terms (adj. p $<$ 0.05)
...
The high weighted entropy (0.9816) and poor unsupervised clustering separation indicate that global gene expression patterns do not cleanly distinguish tumor grades. This suggests that grade-specific differences are concentrated in specific pathways (as shown by improved PCA separation with DEGs only) rather than representing wholesale transcriptional changes. This aligns with emerging concepts of intratumoral heterogeneity and molecular plasticity.
...
\begin{enumerate}
    \item 5,800 high-confidence DEGs identified with DESeq2 (FDR $<$ 0.01, $|\log_2\text{FC}| > 1$)
    \item Clustering validation showing high entropy (0.9816), motivating supervised analysis
    \item 247 enriched GO:BP terms revealing cancer-relevant pathways
