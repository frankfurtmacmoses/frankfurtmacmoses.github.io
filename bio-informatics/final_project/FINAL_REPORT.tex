...
\begin{itemize}
    \item 2,146 DEGs identified with DESeq2 (FDR $<$ 0.01, $|\log_2\text{FC}| > 1$): 800 upregulated and 1,346 downregulated in high-grade tumors
    \item Optimal clustering at k=2 with moderate entropy (0.3902), showing reasonable separation between tumor grades
    \item PC1+PC2 captured 22.77\% variance using all genes, 37.77\% using DEGs only
    \item 153 significant GO biological process terms enriched, highlighting immune response, proliferation, and ECM remodeling
    \item Methodological approach: count-based filtering (genes expressed in $\geq$10\% samples), DESeq2 for proper count data modeling, entropy for cluster quality assessment
\end{itemize}
...
\begin{enumerate}
    \item Identify genes differentially expressed between low-grade and high-grade bladder tumors
    \item Use unsupervised clustering to discover molecular subtypes
    \item Conduct functional enrichment analysis to understand underlying biological mechanisms
    \item Compare findings with published TCGA-BLCA literature
\end{enumerate}
...
Filtering Approach:
\begin{itemize}
    \item Guideline: ``Filter out genes that are not expressed (count $\leq$ 5) in at least 10\% of the samples''
    \item Implementation: Removed genes where count $\leq$ 5 in 9 or more samples (10\% of 90 samples)
    \item Equivalent to: Keeping only genes with count $>$ 5 in at least 90\% of samples
    \item Rationale: This stringent filtering removes lowly expressed genes that contribute noise while retaining biologically relevant transcripts
\end{itemize}

Filtering Results:
\begin{itemize}
    \item Started with: 60,660 genes
    \item Retained: 15,967 genes (26.3\%)
    \item Filtered out: 44,693 genes (73.7\%)
\end{itemize}
...
\begin{itemize}
    \item Input: $\log_2$CPM values for 15,967 genes
    \item Preprocessing: StandardScaler (mean=0, variance=1)
    \item Focus: PC1 and PC2 for visualization
\end{itemize}
...
\begin{itemize}
    \item Model: $K_{ij} \sim \text{NB}(\mu_{ij}, \alpha_i)$ for count data
    \item Handles: Library size differences, gene-specific dispersion, fold change shrinkage
    \item Statistical test: Wald test with Benjamini-Hochberg FDR adjustment
    \item Benefits: Properly models count data, handles overdispersion, improves reliability
\end{itemize}

\subsubsection{Multiple Testing Correction}
We applied Benjamini-Hochberg FDR correction:
\begin{itemize}
    \item Statistical cutoff: FDR $<$ 0.01
    \item Biological cutoff: $|\log_2\text{FoldChange}| > 1$
\end{itemize}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{results/task4_go_enrichment_labeled.png}
\caption{Enhanced GO enrichment dot plot with labeled key terms. Bubble size represents gene count, with yellow-highlighted annotations pointing to major biological processes: Cell Population Proliferation, Cell Migration, Inflammatory Response, and Angiogenesis.}
\label{fig:go_enrichment_labeled}
\end{figure}
...
\begin{enumerate}
    \item Proliferation axis: Faster cell cycle and replication stress
    \item Microenvironment axis: ECM remodeling and immune cell recruitment
\end{enumerate}

\subsubsection{Clinical Implications}
Potential therapeutic targets identified:
\begin{itemize}
    \item Cell cycle inhibitors: Target increased proliferation (e.g., CDK4/6 inhibitors)
    \item MMP inhibitors: Block ECM remodeling and tumor invasion
    \item Immune checkpoint blockade: Take advantage of PD-L1 expression in high-grade tumors
...
\begin{enumerate}
    \item Complete workflow: Combines clustering, differential expression, and pathway enrichment
    \item Solid statistics: FDR correction and multiple quality metrics
    \item Reproducible approach: Set random seeds, documented parameters
    \item Biological context: GO enrichment connected to cancer biology
    \item Literature support: Results align with published TCGA-BLCA studies
\end{enumerate}
...
\begin{enumerate}
    \item Sample size: 90 samples may limit power for finding subtypes
    \item Bulk RNA-seq: Can't distinguish tumor, stromal, and immune cell signals
    \item Missing clinical data: Can't link to survival or treatment response
    \item No validation: Findings not tested in independent datasets
\end{enumerate}

\subsubsection{Future Directions}
\begin{enumerate}
    \item Single-cell RNA-seq: Break down the tumor microenvironment
    \item Multi-omics: Add DNA methylation, copy number, and mutation data
    \item Survival analysis: Connect DEGs to patient outcomes
    \item Lab validation: Test key candidate genes (MMP11, GATA3) functionally
    \item Machine learning: Develop predictive models for grade classification
\end{enumerate}
...
\begin{itemize}
    \item Distinguishes tumor grades in biologically meaningful ways
    \item Points to potential therapeutic targets
    \item Offers biomarkers for prognosis and treatment decisions
    \item Adds to our understanding of bladder cancer biology
\end{itemize}
...
\begin{enumerate}
    \item 2,146 differentially expressed genes found with DESeq2 (FDR $<$ 0.01, $|\log_2\text{FC}| > 1$)
    \item Clustering with moderate entropy (0.3902), showing reasonable grade separation
    \item 153 enriched GO biological process terms pointing to cancer-relevant pathways
    \item Clear visualizations (PCA plots, volcano plots, heatmaps, enrichment plots)
    \item Reproducible analysis pipeline documented in Jupyter notebook
\end{enumerate}

\subsection{Biological Insights:}
\begin{itemize}
    \item High-grade tumors show faster proliferation, ECM remodeling, and loss of differentiation
    \item Molecular heterogeneity suggests need for personalized treatment
    \item Immune signatures point to potential immunotherapy applications
\end{itemize}

\subsection{Clinical Relevance:}
This analysis lays groundwork for:
\begin{itemize}
    \item Discovering biomarkers to predict tumor grade
    \item Identifying therapeutic targets (MMP11, PD-L1, CDKs)
    \item Stratifying patients for precision medicine
\end{itemize}
